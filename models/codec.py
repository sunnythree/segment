...
class BasicBlockT(nn.Module):
    def __init__(self, inplanes, planes, stride=1, outpadding=0):
        ...
        self.conv1 = conv3x3t(inplanes, planes, stride, outpadding)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU()
        self.stride = stride
...
class CodecNet(nn.Module):
    def __init__(self, layers):
        ...
        self.layer_t4 = self._make_layer(BasicBlockT, 22, layers[7], stride=2, outpadding=1)
...
def CodecNet13():
    model = CodecNet([1, 1, 1, 1, 1, 1, 1, 1])
    return model
